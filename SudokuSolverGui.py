...
def format_time(secs):  # this function convert sec in to min and round up sec
    sec = secs % 60
    minute = secs // 60
    mat = " " + str(minute) + ":" + str(sec) #str used for convertin into string
    return mat


def drawClock():
    pygame.draw.line(screen, BLACK, (0, width), (width, width)) #pygame's function
...
def drawGrid():                                              # draw all grid and clock
    cellSize = int(width / 9)                                # cell 1/9 0f width = 60
...
def fillBoard(grid):  # this function fills board with number in our board matrix
    ...
    for i in range(len(grid)):                      # standard double loop
        for j in range(len(grid[0])):
            ...
            #blit is a thin wrapper around pygame whick allows you to easily draw images to the screen
...
def makeItRed(cell_num,key):                                # mark the selected cell with red border and fill temp key
    if board[cell_num[0]][cell_num[1]] == 0:                # only cell with zero in it are selected
        ...
        x2 = x1 + 60                                        #cellnum[1] meaning x coordinate X1 is start and x2 is end of cell
        y1 = cell_num[0] * 60                               #cellnum[2] meaning y coordinate same with y1 & y2
        y2 = y1 + 60
        pygame.draw.line(screen, RED, (x1, y1), (x1, y2))    #drawing red lines to all 4 sides of that cell
...
def solver(bo):                          # recursive solution of problem
    ...
    return False                         # return that no solution exit which will never be the case in out program
...
def solverSpecial(grid, position, key):       # driver function to check that a num at a position will give solution
    ...
    return solver(arr)                        # returning if solution exist
